...
public class DiagonalDisproportion
{
    public int getDisproportion(string[] matrix)
    {
        ...
        for (int i = 0; i < l; i++)
        {
            md += int.Parse(matrix[i][i].ToString());
            cd += int.Parse(matrix[i][l - i - 1].ToString());
        }
        return md - cd;
    }

    // CUT begin
    static bool DoTest(string[] matrix, int __expected)
    {
        ...
        try
        {
            __result = instance.getDisproportion(matrix);
        }
        catch (Exception e)
        {
            exception = e;
    ...
    public static void Main(string[] args)
    {
        ...
        Console.Error.WriteLine("DiagonalDisproportion (250 Points)");
        Console.Error.WriteLine();

        int nCases = 0, nPassed = 0;
        using (var reader = File.OpenText("DiagonalDisproportion.sample"))
        {
            while (true)
            {
                ...
                string[] matrix = new string[int.Parse(reader.ReadLine())];
                for (int i = 0; i < matrix.Length; ++i)
                    matrix[i] = (string)Convert.ChangeType(reader.ReadLine(), typeof(string));
                reader.ReadLine();
        ...
        int T = (int)((DateTime.UtcNow - Jan1st1970).TotalSeconds - 1409495533);
