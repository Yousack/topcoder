...
public class CheckFunction
{
    public int newFunction(string code)
    {
        ...
        {
            sum += dashes[int.Parse(c.ToString())];
        }
        return sum;
    }

    // CUT begin
    static bool DoTest(string code, int __expected)
    {
        ...
        try
        {
            __result = instance.newFunction(code);
        }
        catch (Exception e)
        {
            exception = e;
    ...
    public static void Main(string[] args)
    {
        ...
        Console.Error.WriteLine("CheckFunction (250 Points)");
        Console.Error.WriteLine();

        int nCases = 0, nPassed = 0;
        using (var reader = File.OpenText("CheckFunction.sample"))
        {
            while (true)
            {
                ...
                string code = (string)Convert.ChangeType(reader.ReadLine(), typeof(string));
                reader.ReadLine();
        ...
        int T = (int)((DateTime.UtcNow - Jan1st1970).TotalSeconds - 1409561765);
